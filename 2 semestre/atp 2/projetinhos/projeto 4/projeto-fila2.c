...
void addWork(struct queue *header, struct OS *reception, int num_SC)
{
    int i;
    struct queue *aux = header;
    while (aux != NULL && aux->time <= execution_time)
    {
        i  = 0;
        // verifica se pode adicionar
        if (aux->attending == 0)
        {
            while(reception[i].busy == 0 && i < num_SC)
                i++;
            if (i == num_SC -1 )
                return; //significa que nao tem nenhum lugar disponivel 
            else
            {
                reception[i].busy = 1;
                reception[i].job = aux;
                aux->attending = 1;
                aux->final_time = aux->weight + execution_time;
            }
        }
        aux = aux->next;
    }
